...
const Foods = () => {
  ...
  const API_URL = 'http://192.168.1.2:3000/api/admin/foods';
  ...
  const fetchBrands = async () => {
    try {
      const res = await axios.get('http://192.168.1.2:3000/api/admin/brands');
  ...
  const getFoodImagePath = (dbImage: string, brandId: number) => {
    ...
    if (img.includes('food-')) {
      ...
      return `http://192.168.1.2:3000/public/foods/${filename}`;
    }

    if (img.startsWith('/public/foods/')) return `http://192.168.1.2:3000${img}`;
