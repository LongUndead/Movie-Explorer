  ...
  class CartManager extends ChangeNotifier {
    ...
    // ========================================================
  // 🚀 THÊM HÀM NÀY ĐỂ NHẬN SỐ PHÚT TỪ API ADMIN GỬI XUỐNG
  // ========================================================
  void updateHoldTimeConfig(int minutes) {
    int newSeconds = minutes * 60;
    
    // Nếu Admin có đổi số phút khác với số hiện tại trong hệ thống
    if (systemHoldSeconds != newSeconds) {
      systemHoldSeconds = newSeconds; // Cập nhật gốc
      
      // CHỈ CẬP NHẬT ĐỒNG HỒ NẾU GIỎ HÀNG ĐANG TRỐNG
      // (Để tránh việc khách đang mua vé dở 5 phút tự nhiên bị reset giờ)
      if (tickets.isEmpty && foods.isEmpty) {
        holdSeconds = systemHoldSeconds;
        notifyListeners();
      }
    }
  }
    ...
    // 🚀 LẤY TÊN RẠP HIỆN TẠI ĐANG CÓ TRONG GIỎ HÀNG
    String? get currentCinemaName {
      if (tickets.isNotEmpty) return tickets.first.cinemaName;
      if (foods.isNotEmpty) return foods.first.cinemaName;
      return null;
    }

    // 🚀 KIỂM TRA XEM MÓN ĐỊNH THÊM VÀO CÓ CÙNG RẠP KHÔNG
    bool canAddItem(String newCinema) {
      final current = currentCinemaName;
      if (current == null) return true; // Giỏ trống -> Thoải mái thêm
      return current.trim().toLowerCase() == newCinema.trim().toLowerCase();
    }
    ...
    void updateCart({
      ...
      if (seats.isEmpty) {
        ...
        expireTime = DateTime.now().add(Duration(seconds: systemHoldSeconds));
      }
  ...
  // =====================================================
    // 🚀 HÀM HỖ TRỢ XỬ LÝ ẢNH DÙNG CHUNG CHO GIỎ HÀNG
    // =====================================================
    ...
    // =====================================================
    // 🚀 HÀM KIỂM TRA ĐƯỜNG DẪN ẢNH BẮP NƯỚC CHO GIỎ HÀNG
    // =====================================================
    String _getCartFoodImagePath(Food food) {
      ...
      // 3. Ảnh Local Assets có sẵn trong App
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ClipRRect(
                                      ...
                                      // 🚀 ĐÃ GỌI HÀM XỬ LÝ ẢNH PHIM THÔNG MINH
                                      child: Image.network(
                                        _getImage(item.movie?.posterPath), 
                            ...
                            child: SwipeableCartItem(
                              ...
                              child: Container(
                                ...
                                child: Row(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    // 🚀 ĐÃ GỌI HÀM VẼ ẢNH BẮP NƯỚC THÔNG MINH KẾT HỢP XỬ LÝ PATH
                                    ClipRRect(
                                      borderRadius: BorderRadius.circular(8),
                                      child: _buildFoodImage(_getCartFoodImagePath(item.food), width: 70, height: 70),
                                    ),
                                    const SizedBox(width: 12),
                                    // Thông tin bắp nước
                                    Expanded(
